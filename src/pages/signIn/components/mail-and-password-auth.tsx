  ...
  const handleEmailPasswordLogin = async () => {
    ...
    try {
      ...
      if (res.result === 'success') {
        if (isInvite) {
          history.replace(`/signin/invite-settings?${searchParams.toString()}`)
        }
        else {
        ...
        if (allowRegistration) {
          ...
          history.replace(`/reset-password/check-code?${params.toString()}`)
        }
        else {
          Toast.notify({
            type: 'error',
    ...
    <div className='mb-3'>
      <label htmlFor="password" className="my-2 flex items-center justify-between">
        ...
        <a
          ...
          aria-disabled={!isEmailSetup ? 'true' : 'false'}
          onClick={(e) => {
            if (!isEmailSetup) {
              e.preventDefault()
            }
