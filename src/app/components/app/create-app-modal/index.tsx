...
function AppPreview({ mode }: { mode: AppMode }) {
    ...
    return <div className='px-8 py-4'>
        ...
        <div className='system-xs-regular mt-1 min-h-8 max-w-96 text-text-tertiary'>
            ...
            {previewInfo.link && <a target='_blank' rel='noopener noreferrer' href={previewInfo.link} className='ml-1 text-text-accent'>{t('app.newApp.learnMore')}</a>}
        </div>
    </div>
}
...
function CreateApp({ onClose, onSuccess, onCreateFromTemplate }: CreateAppProps) {
    ...
    useEffect(() => {
        // umi 的 useSearchParams 返回 [params]，需解构
    ...
    const onCreate = useCallback(async () => {
        ...
        try {
            ...
            getRedirection(isCurrentWorkspaceEditor, app, navigate)
        }
        catch {
            notify({ type: 'error', message: t('app.newApp.appCreateFailed') })
        }
        isCreatingRef.current = false
    }, [name, notify, t, appMode, appIcon, description, onSuccess, onClose, mutateApps, navigate, isCurrentWorkspaceEditor])
